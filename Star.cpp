...
Star::Star(int layer) : Texture("Stars.png",0,0,4,4) {
	...
	Position(Vector2((float)(mRandom->RandomInt() % Graphics::Instance()->SCREEN_WIDTH), (float)(mRandom->RandomInt() % Graphics::Instance()->SCREEN_HEIGHT)));
...
void Star::ScrollStar() {
	...
	Translate(Vect2_Up * mScrollSpeed);
...
void Star::Render() {
	...
	if (mVisible) {
		// we can do this because we enherit from texture and we have overwriten the render funciton.
		Texture::Render();
	}
